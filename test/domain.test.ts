...
test("accepts a valid configuration with typed references", () => {
  const config: AdaptationConfig = {
    ...
    content: [
      { kind: "actor", key: contentKey("dead-suns.ch01.security-robot"), name: "Security Robot" },
      { kind: "scene", key: contentKey("dead-suns.ch01.docking-bay-94"), name: "Docking Bay 94" },
      {
        kind: "encounter",
        key: contentKey("dead-suns.ch01.docking-bay-94.encounter"),
        name: "Docking Bay Ambush",
        scene: { kind: "scene", key: contentKey("dead-suns.ch01.docking-bay-94") },
        actors: [{ kind: "actor", key: contentKey("dead-suns.ch01.security-robot") }],
      },
    ],
  };
  ...
  const config: AdaptationConfig = {
    ...
    content: [
      ...
      { kind: "scene", key: duplicatedKey, name: "Second" },
      {
        ...
        scene: { kind: "scene", key: contentKey("dead-suns.ch01.missing-scene") },
...
test("rejects keys that are not stable slugs", () => {
  assert.throws(() => contentKey("Dead Suns/Chapter 1"), /Invalid content key/);
});
